...
class AppointmentController extends Controller
{
    public function index(Request $r)
    {
        ...
        if ($r->filled('doctor_id')) $query->where('doctor_id', $r->doctor_id);
        if ($r->filled('patient_id')) $query->where('patient_id', $r->patient_id);
    ...
    public function store(StoreAppointmentRequest $r)
    {
        ...
        if ($slot->is_booked) return response()->json(['message' => 'Slot already booked'], 422);
